...
def suture(filepath, column_threshold, row_threshold):
    ...
    if proceed.lower() == 'y':
        fileOutput_name = input("\nEnter a name for the new file (DO NOT INCLUDE .CSV EXTENSION): ")
...
def doubleCheck():
    """
    A contingency protocol, asks user to provide input in the form of a file pathway, then loads said file and checks
    if the file in question contains any NaN values.

    :return: Boolean statement
    """

    print("\n\n- CONTINGENCY ANALYSIS -")
    print("This protocol, if initiated, will check if a given data frame contains NaN values")
